...
def patch_server() -> None:
  ...
  if (!order) {''',
    ...
    text = replace_once(
      ...
      calls: postgresCalls,
      postgres: postgresResult,''',
        "delta response calls",
    )

    call_helpers = '''function jerusalemCalendarNow() {
...
'''
    text = replace_once(text, "async function handleCustomerOrder(payload, req, res) {", call_helpers + "async function handleCustomerOrder(payload, req, res) {", "call date helpers")
  ...
  return handleOrderDelta({
    ...
    calls: callRows,
        ...
        "customer portal call cycle",
    )

    merge_handler = '''async function handleOrdersMerge(payload, res) {
...
'''
    text = replace_once(text, "function handleStatic(req, res) {", merge_handler + "function handleStatic(req, res) {", "merge handler")
...
def patch_management() -> None:
    path = Path("management/app.js")
    text = path.read_text(encoding="utf-8")
  ...
  bindPickingActions();''',
    ...
    merge_client = '''async function mergePickingOrders(targetOrderId, sourceOrderId) {
...
'''
    text = replace_once(text, "function normalizeClosedOrderStatuses() {", merge_client + "function normalizeClosedOrderStatuses() {", "merge client function")
...
patch_server()
patch_management()
print("Sales operations patch applied successfully")
